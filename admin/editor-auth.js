...
const EditorAuth=(()=>{
  ...
  async function ensure(){
    const tk=await token();const r=await fetch('/api/access/me',{headers:{authorization:'Bearer '+tk},cache:'no-store'});const d=await r.json();
  ...
  async function api(url,options={}){
    ...
    const r=await fetch(url,{...options,headers,cache:'no-store'});const d=await r.json().catch(()=>({}));
    if(!r.ok)throw new Error(d.message||`Fehler ${r.status}`);return d;
  }
  return {init,ensure,api,get access(){return access}};
})();
