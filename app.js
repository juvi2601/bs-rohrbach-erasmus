...
function renderMap(places){
  ...
  const osmUrl=(bbox,marker)=>{
    const box=bbox.map(n=>Number(n).toFixed(6)).join('%2C');
    const pin=marker?`&marker=${Number(marker.lat).toFixed(6)}%2C${Number(marker.lng).toFixed(6)}`:'';
    return `https://www.openstreetmap.org/export/embed.html?bbox=${box}&layer=mapnik${pin}`;
  };
  ...
  function renderMapFrame(index=activeIndex){
    ...
    const bbox=bboxAround(Number(p.lat),Number(p.lng));
    canvas.innerHTML=`<div class="travel-map-frame"><iframe title="Interaktive Karte – ${esc(p.title)}" src="${osmUrl(bbox,p)}" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe><div class="travel-map-label"><span style="--pin:${styleFor(p.category).color}">${esc(styleFor(p.category).icon)}</span><div><small>Aktuell ausgewählt</small><strong>${esc(p.title)}</strong></div></div></div>`;
  }
