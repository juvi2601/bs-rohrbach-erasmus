...
async function init(){
  ...
  fetchJson("/api/media/gallery?trip=bruessel-2026").then(r=>{const approved=Array.isArray(r?.items)?r.items.map(x=>({...x,_studentPhoto:true})):[];if(approved.length){galleryData=[...approved,...galleryData];renderGallery(galleryData)}}).catch(()=>{});
...
function applySite(site){
  ...
  const tick=()=>{
    ...
    if(state?.phase==="during"||nowMs>=target){
      const programDay=(programData.days||[]).find(d=>d.id===state?.day?.programId);
...
function getJourneyPreview(){
  try{const raw=localStorage.getItem(JOURNEY_PREVIEW_KEY);if(!raw)return null;const value=JSON.parse(raw);return value&&value.enabled&&value.date?value:null}catch{return null}
}
function getJourneyNow(){
  const preview=getJourneyPreview();
  if(!preview)return new Date();
  const simulated=new Date(`${preview.date}T12:00:00+01:00`);
  return Number.isNaN(simulated.getTime())?new Date():simulated;
}
function renderJourneyPreviewNotice(){
  const preview=getJourneyPreview();if(!preview)return;
  const bar=document.createElement("div");bar.className="journey-preview-notice";bar.setAttribute("role","status");
  bar.innerHTML=`<strong>🧪 Smart-Journey-Vorschau</strong><span>Simuliertes Datum: ${preview.date.split("-").reverse().join(".")}</span><button type="button">Vorschau beenden</button>`;
  bar.querySelector("button").addEventListener("click",()=>{localStorage.removeItem(JOURNEY_PREVIEW_KEY);location.reload()});
...
function currentJourneyState(journey,days){
  if(!journey?.enabled)return null;const now=getJourneyNow(),key=localDateKey(now),start=new Date(journey.trip?.start),end=new Date(journey.trip?.end);
...
function renderDiary(entries){
  ...
  const card=(e,featured=false)=>{const approved=(e.approvedPhotos||[]).map(x=>typeof x==='string'?{image:x}:x).filter(x=>x.image);const legacy=(e.images||[]).map(x=>typeof x==='string'?{image:x}:x).filter(x=>x.image);const images=[...approved,...legacy];const cover=e.cover||images[0]?.image||"";const galleryImages=cover?images.filter((im,i)=>!(i===0&&im.image===cover)):images;const tags=(e.tags||[]).map(t=>typeof t==='string'?t:t?.tag).filter(Boolean);return `<article class="diary-card ${featured?'featured':''}">${cover?`<button class="diary-cover" type="button" data-diary-image="${esc(cover)}" aria-label="${esc(e.title||'Foto')} vergrößern"><img src="${esc(cover)}" alt="${esc(e.alt||e.title||'Foto aus dem Reisetagebuch')}" loading="lazy"></button>`:''}<div class="diary-card-body"><div class="diary-meta"><span>${esc(e.emoji||'📖')}</span><time>${formatDate(e.date)}${e.time?' · '+esc(e.time):''}</time>${e.location?`<span>📍 ${esc(e.location)}</span>`:''}</div><h3>${esc(e.title||'Tagebucheintrag')}</h3><p class="diary-text">${esc(e.text||'').replace(/\n/g,'<br>')}</p>${tags.length?`<div class="diary-tags">${tags.map(t=>`<span>${esc(t)}</span>`).join('')}</div>`:''}${galleryImages.length?`<div class="diary-gallery">${galleryImages.map((im,i)=>`<button type="button" data-diary-image="${esc(im.image)}" aria-label="Weiteres Foto ${i+1} vergrößern"><img src="${esc(im.image)}" alt="${esc(im.alt||e.title||'Weiteres Reisefoto')}" loading="lazy"></button>`).join('')}</div>`:''}${e.author?`<small class="diary-author">Redaktion: ${esc(e.author)}</small>`:''}</div></article>`};
  latest.hidden=false;latest.innerHTML=`<p class="diary-new-label">🆕 Neuester Eintrag</p>${card(rows[0],true)}`;
  const older=rows.slice(1);timeline.id='diaryAll';timeline.innerHTML=older.length?`<h3 class="diary-more-title">Weitere Einträge</h3>${older.map(e=>card(e,false)).join('')}`:'';timeline.hidden=!older.length;
...
function renderToday(days,journey=journeyData){
  ...
  setText("progressLabel",label);setText("progressDate",dateText);
}
...
function renderLegalLinks(items){
  ...
  wrap.innerHTML=items.map(x=>`<button type="button" data-legal="${esc(x.panel)}">${esc(x.label)}</button>`).join("")+`<a class="footer-admin-link" href="/admin/login.html?trip=bruessel-2026" aria-label="Administratorbereich öffnen">Administrator</a>`;
