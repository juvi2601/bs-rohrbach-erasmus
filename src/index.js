...
export default {async fetch(request,env){
  const url=new URL(request.url);
    ...
    if((url.pathname==='/linz-2027/upload'||url.pathname==='/linz-2027/upload/')&&request.method==='GET'){
      ...
      const asset=await env.ASSETS.fetch(new Request(`${url.origin}/upload.html?v=14.0-dev.10.0.2`,{
    ...
    if((url.pathname==='/linz-2027'||url.pathname==='/linz-2027/')&&request.method==='GET'){
      const meta=await getPublishedMeta(env,'linz-2027');
      if(meta?.published){
        const asset=await env.ASSETS.fetch(new Request(`${url.origin}/reise.html?v=14.0-dev.10.0.2`,{method:'GET',headers:{'cache-control':'no-cache'}}));
        const headers=new Headers(asset.headers);
        headers.set('cache-control','no-store, max-age=0');
        headers.set('pragma','no-cache');
    ...
    if(url.pathname==='/api/trips/public-health'&&request.method==='GET'){
      return json({ok:true,version:'14.0-dev.10.0.2',statusHelper:'ok'},200,{'x-bsr-health':'9.1.3'});
    }
