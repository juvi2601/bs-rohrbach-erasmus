...
export default {async fetch(request,env){
  const url=new URL(request.url);
    ...
    if((url.pathname==='/bruessel-2026'||url.pathname==='/bruessel-2026/')&&request.method==='GET'){
      ...
      const asset=await env.ASSETS.fetch(new Request(`${url.origin}/bruessel.html?v=15.0.8-dev`,{method:'GET',headers:{'cache-control':'no-cache'}}));
      const headers=new Headers(asset.headers);
      headers.set('cache-control','no-store, max-age=0');
      headers.set('pragma','no-cache');
    ...
    if(url.pathname==='/api/trips/public-health'&&request.method==='GET'){
      return json({ok:true,version:'15.0.8-dev',statusHelper:'ok'},200,{'x-bsr-health':'9.1.3'});
    }
